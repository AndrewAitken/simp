        ...
        <DrawerContent className="max-w-full sm:max-w-md mx-auto">
          <div className="p-4">
                ...
                <div className="flex items-center justify-center text-5xl">
                  <div className="flex flex-col items-center mx-2">
                    <button 
                      ...
                      onClick={() => handleHourChange(displayHour + 1)}
                    >
                      {displayHour === 12 ? 1 : displayHour + 1}
                    </button>
                    <button 
                      className="text-black text-6xl font-bold my-4"
                    >
                      {String(displayHour).padStart(2, "0")}
                    </button>
                    <button 
                      className="text-gray-300 hover:text-gray-600" 
                      onClick={() => handleHourChange(displayHour - 1 || 12)}
                    >
                  ...
                  <div className="ml-6">
                    <button 
                      className={cn(
                        "block w-20 py-2 text-xl rounded-md mb-2", 
                        period === "AM" ? "bg-gray-100 font-bold" : "text-gray-400"
                      )}
                      onClick={() => period !== "AM" && handlePeriodToggle()}
                    >
                      AM
                    </button>
                    <button 
                      className={cn(
                        "block w-20 py-2 text-xl rounded-md", 
                        period === "PM" ? "bg-gray-100 font-bold" : "text-gray-400"
                      )}
                      onClick={() => period !== "PM" && handlePeriodToggle()}
                    >
                      PM
                    </button>
                  </div>
            ...
            <div className="flex justify-between">
              <Button variant="outline" onClick={() => setIsOpen(false)} className="w-[45%] py-6 rounded-xl">
                Cancel
              </Button>
              <Button 
                onClick={handleSave} 
                className="w-[45%] bg-sky-400 hover:bg-sky-500 text-white py-6 rounded-xl"
              >
                Save
              </Button>
            </div>
